  ...
  var req = https.request(options, function(res) {
    res.on('data', buf => {
      try {
        ...
        if (obj.error) {
          console.error(`[${img}]：压缩失败！报错：${obj.message}`);
        } else {
          fileUpdate(img, obj);
        }
      } catch (error) {
        console.error(`[${img}]：压缩失败！`);
      }
    });
  });
...
function fileUpdate(imgpath, obj) {
  ...
  const outputPath = path.join(cwd, 'output', imgpath.replace(cwd, ''));
  ...
  const req = https.request(options, res => {
    ...
    res.on('end', function() {
      fs.writeFile(outputPath, body, 'binary', err => {
        ...
        // 压缩完成的移入done目录
        const donePath = path.join(cwd, 'done', imgpath.replace(cwd, ''));
        fs.renameSync(imgpath, donePath);
      });
    });
  });
  req.on('error', e => {
